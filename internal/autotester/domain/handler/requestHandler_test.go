...
import (
	...
	service_intf "gitlab.dit.htwk-leipzig.de/projekt2025-w-llm-unterstuetztes-autotesting-fuer-moderne-web-frontends/smart/internal/autotester/domain/service"
...
func TestNewAutoTesterController(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.testName, func(t *testing.T) {
			controller, err := NewAutotesterController(test.logger, test.config, mockValServ, mockGenServ, tracer)
...
func TestHandleChatRequest(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.TestName, func(t *testing.T) {
			...
			controller, err := NewAutotesterController(logger, cfg, mockValServ, mockGenServ, tracer)
...
func TestHandleError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "validate error",
			givenError:  service_intf.ErrNotEnoughInformation,
			wantedError: service_intf.ErrNotEnoughInformation,
			wantErr:     true,
		},
	}
