...
func NewValidatePromptService(service *service.OpenAIService, config *config.Config, logger *slog.Logger) (*ValidatePromptService, error) {
	if err := assert.NotNil(service, config, logger); err != nil {
		return nil, err
	}
	return &ValidatePromptService{service, config, logger}, nil
}
...
func (s *ValidatePromptService) ValidatePrompt(ctx context.Context, userPrompt string, sessionID string) error {
	req := entity.Request{
		Prompt:       userPrompt,
		SessionID:    sessionID,
